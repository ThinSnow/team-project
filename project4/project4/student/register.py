...
from common.models import Student, Administer,StudentScore
import json
...
def dispatcher(request):
    ...
    try:
        ...
        elif action == 'modify_students':
            return modifystudents(request)
        elif action == 'del_students':
...
def liststudents(request):
    """列出所有学生"""
    try:
        students = Student.objects.all().values('id', 'password')  # 避免返回敏感字段
...
def addstudents(request):
    ...
    try:
        ...
        # 检查学生是否已存在
        if Student.objects.filter(id=info['id']).exists():
            return JsonResponse({'ret': 1, 'msg': '8学号已存在'})

        # 创建新学生
        student = Student.objects.create(id=info['id'], password=info['password'])
        StudentScore.objects.create(student=student)
